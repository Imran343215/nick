...
export default function RepairServicesManager() {
  ...
  useEffect(() => {
    Promise.all([loadBrands(), loadAllDevices(), loadServices()])
  ...
  const formDevices = useMemo(
    () =>
      form.brand
        ? allDevices.filter((d) => d.brand === form.brand)
        : [],
  ...
  function startEdit(service: RepairServiceShape) {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    const res = await fetch(
      editingId ? `/api/admin/repair-services/${editingId}` : "/api/admin/repair-services",
      {
  ...
  return (
    <AdminShell
      ...
      lead="Set prices and turnaround for each repair option on a device."
    >
      {error && <div className="alert alert--error">{error}</div>}

      <form className="form-card admin-form-card" onSubmit={handleSubmit}>
        <h2 className="admin-card-title">{editingId ? "Edit service" : "Add service"}</h2>
        <div className="form-grid">
          <div className="field">
            ...
            <select
              ...
              value={form.brand}
              onChange={(e) =>
                setForm({ ...form, brand: e.target.value, device: "" })
              }
            >
          ...
          <div className="field field--full">
            ...
            {form.icon && (
              <img src={form.icon} alt="Service icon preview" className="catalog-admin-thumb" />
            )}
          </div>
        </div>
        <div className="form__actions">
          <button className="btn btn--primary" type="submit" disabled={uploading}>
            {uploading ? "Uploading..." : editingId ? "Save changes" : "Add service"}
          </button>
          {editingId && (
      ...
      <div className="admin-panel">
        <div className="admin-panel__head">
          <h2 className="admin-card-title">All services</h2>
        ...
        <DataTable
          loading={loading}
          emptyMessage="No repair services yet — add one above."
